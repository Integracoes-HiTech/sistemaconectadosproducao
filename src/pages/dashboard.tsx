...
export default function Dashboard() {
  ...
  const userIdFilter = user?.id; // Sempre usar o ID do usuário logado para links
  ...
  const generateLink = async () => {
    console.log('🔍 Debug - generateLink chamada');
    console.log('🔍 Debug - user:', user);
    console.log('🔍 Debug - user.id:', user?.id);
    console.log('🔍 Debug - user.full_name:', user?.full_name);
    ...
    console.log('🔍 Debug - Chamando createLink com:', { userId: user.id, userName: user.full_name });
    const result = await createLink(user.id, user.full_name);
    console.log('🔍 Debug - Resultado createLink:', result);
    ...
    if (result.success && result.data) {
      ...
      if (result.data.is_existing) {
        toast({
          title: "✅ Link copiado!",
          description: "Seu link existente foi copiado para a área de transferência.",
        });
      } else {
        toast({
          title: "🎉 Link gerado!",
          description: "Novo link foi gerado e copiado para a área de transferência.",
        });
      }
    } else {
      console.log('❌ Debug - Erro no resultado:', result);
      toast({
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            {isAdminUser && (
              <Button
                onClick={() => {
                  try {
                    ...
                    // Exportar dados completos do relatório (igual aos cards do dashboard)
                    ...
                    toast({
                      ...
                      description: "Arquivo PDF com dados completos do relatório foi baixado com sucesso!",
                    });
                  } catch (error) {
                    toast({
              ...
              >
                ...
                Exportar Dados do Relatório (PDF)
              </Button>
            )}
              ...
              <div>
                ...
                <p className={`${textColor.replace('800', '700')} text-sm`}>
                  {isExceeded ? (
                    ...
                    <>
                      O sistema está próximo do limite de {limitStatus.max_members} membros. 
                      Atualmente temos {limitStatus.member_count} membros cadastrados 
                      ({limitStatus.percentage.toFixed(1)}% do limite).
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-institutional-blue">
                          {member.ranking_position && member.ranking_position > 0 ? `${member.ranking_position}º` : 'N/A'}
                        </span>
                        {member.is_top_1500 && (

-- ALTERAÇÕES PARA MELHORAR O AUTOCOMPLETE
-- Execute apenas estas alterações no Supabase SQL Editor

-- 1. Criar tabelas se não existirem (caso não tenham sido criadas ainda)
...
-- 4. Função melhorada para buscar cidades (substitui a anterior)
...
-- 5. Função melhorada para buscar setores (substitui a anterior)
...
-- 6. Inserir municípios principais de Goiás (apenas se não existirem)
...
-- 7. Inserir setores comuns em Goiânia (apenas se não existirem)
...
-- 8. Criar índices para melhorar performance (apenas se não existirem)
...
-- 10. Criar políticas RLS se não existirem
DO $$
BEGIN
